    ...
    <div className="reveal-scale flex justify-center gap-3 sm:gap-5 mb-16">
      {units.map((u, i) => (
        <div key={i} className="flex flex-col items-center">
          <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-white/70 backdrop-blur-md border border-white/60 shadow-lg flex items-center justify-center soft-aura">
            <span suppressHydrationWarning className="font-display text-2xl sm:text-3xl text-rose-600 font-semibold tabular-nums">
              {u.value}
            </span>
          </div>
...
function EventCard({
  ...
  return (
    <div className={`${revealClass} bg-white/85 backdrop-blur-md border border-white/50 shadow-[0_10px_40px_rgba(0,0,0,0.08)] rounded-[32px] p-8 transition-transform duration-500 hover:scale-[1.03] group soft-aura`}>
      <div className="flex justify-center mb-6">
        <FloralFrame size={70} className="animate-float opacity-80" />
      </div>

      <h3 className="font-script text-4xl text-rose-600 text-center mb-6">{event.title}</h3>
